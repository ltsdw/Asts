...
class CardsGenerator(Thread):
    def __init__(
        ...
        :param cards_editor_state: Class to keep track and update the state of CardsEditor class.
    ...
    def _cut_medias(self) -> None:
        """
        _cut_medias

        Cut the clip selected to be used at the creation of cards.

        :return: A list with the futures generated by this function jobs.
        """

        for card_info_list in self._chunks_card_info_list:
            with ThreadPoolExecutor(max_workers=self._max_workers) as executor:
                for card_info in card_info_list:
                    if not self._cards_editor_state.is_state(CardsEditorStates.RUNNING): return
